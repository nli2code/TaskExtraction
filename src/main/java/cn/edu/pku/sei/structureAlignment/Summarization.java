...
//        tryToMatchNonleafNode();
    ...
    public static void main(String[] args) {
        ...
        String description1 = "I need to add table to existing docx document and then convert it to Pdf file, so Im using Apache POI and Apache POI converter libs.\n" +
                "There is my code:\n" +
                ...
                "So I cant figure out why I receive \"<em>The number of columns in PdfPTable constructor must be greater than zero.</em>\" exception, when my created table has 3 rows and 3 columns.\n" +
                "Looks like some issue with pdf converter or I create table in the wrong way.\n" +
                "Maybe someone could suggest something to me? \n";
        String description2 = "Pdf file";
        ...
        String code2 = "Hits hits = searcher.search(query);" +
                "int offset = page * recordsPerPage;" +
                "int count = Math.min(hits.length() - offset, recordsPerPage);" +
                "for (int i = 0; i < count; ++i) {" +
                "  Document doc = hits.doc(offset + i);" +
                "  " +
                "}";
        Summarization summarization = new Summarization(code2, description);
